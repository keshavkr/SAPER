/* $Header: oimcp/connectors/sap/src/main/oracle/iam/connectors/sap/hrms/tasks/SAPHRMSLookupRecon.java /main/18 2016/07/11 22:08:47 srkale Exp $ */
...
/**
 *  @version $Header: oimcp/connectors/sap/src/main/oracle/iam/connectors/sap/hrms/tasks/SAPHRMSLookupRecon.java /main/18 2016/07/11 22:08:47 srkale Exp $
 *  @author  ddkumar 
...
/**
 * Description: Mapped to the scheduled task that is configured for employee type lookup reconciliation
 */
public class SAPHRMSLookupRecon extends SchedulerBaseTask implements
		HRMSConstants {
	...
	private tcSchedulerOperationsIntf schedulerAPI;
	...
	public void init() {
		...
		try {
			logger.info(className, sMethodName,
					"Start of Lookup Reconciliation process");
			...
			OIMUtil oUtil = new OIMUtil(userAPI, objAPI, formAPI, formDefAPI, resAPI,
	...
	/**
	 *Description: Called each time the scheduled task runs and executes the lookup reconciliation process
	 */
	public void execute() {
		...
		try {
			if (isValid) {
				HashMap hmLookupValues = null;
				jcoConnection = sAPConnection.addDestination(htITRMapping,
						htITRattributes);
				hmLookupValues = getDropDownValues(jcoConnection);
				if (hmLookupValues.size() > 0) {
					addLookupvalues(lookIntf, sLookupName, hmLookupValues);
				}
			} else {
				/*  Start Bug 23312345 : ER LOOKUP SHOWING THE SUCCESS STATUS WHEN IT FAILED */
			...
			try {
				...
				logger.setMethodFinishLog(className, sMethodName);
				logger.info("End of SAP Lookup Reconciliation process");
			} catch (ConnectorException e) {
				logger.error(className, sMethodName, "ConnectorException");
				logger.setStackTrace(e, className, sMethodName, e.getMessage());				
			} catch (Exception e) {
	...
	/**
	 * Description : This method executes the BAPI DESCRIPTION_FOR_HELPVALUES to
	 * get all the employee group and sub group values
	 * 
	 * @param jcoConnection
	 *          JCoDestination value to connect to SAP
	 * @throws ConnectorException
	 */
	private HashMap getDropDownValues(JCoDestination jcoConnection)
			throws ConnectorException {
		HashMap returnValuesMap = new HashMap();
		String sMethodName = "getDropDownValues()";
		try {
			logger.setMethodStartLog(className, sMethodName);
			String sFieldNames[] = ((String) hmConstants
					.get(LOOKUP_BAPI_STRUC_FIELD_NAME)).split(";");
			String sFieldValues[] = ((String) hmConstants
					.get(LOOKUP_BAPI_STRUC_FIELD_VALUE)).split(";");
			// calls getdropDownListFunction to get JCoFunction
			...
			JCoStructure jcoStructure = jcoFunction.getImportParameterList()
					.getStructure((String) hmConstants.get(LOOKUP_BAPI_STRUCTURE));
			logger.debug(className, sMethodName, "Structure: " + hmConstants.get(LOOKUP_BAPI_STRUCTURE));
			jcoStructure.setValue(sFieldNames[0], sFieldValues[0]);
			logger.debug(className, sMethodName, sFieldNames[0] + " set to " + sFieldValues[0]); 
			jcoStructure.setValue(sFieldNames[1], sFieldValues[1]);
			logger.debug(className, sMethodName, sFieldNames[1] + " set to " + sFieldValues[1]);
			jcoFunction.execute(jcoConnection);
			returnValuesMap = getDropValues(jcoFunction);
		} catch (Exception e) {
			logger.error(className, sMethodName, "Exception");
			logger.setStackTrace(e, className, sMethodName, e.getMessage());
	...
	/**
	 * Description : This method gets the dropdownvalues of usertype in <employee
	 * group> ~ <employee subgroup> format.
	 * 
	 * @param dropDownListFunction
	 *          JCoFunction recieved after executing BAPI
	 * @throws ConnectorException
	 */
	private HashMap getDropValues(JCoFunction dropDownListFunction)
			throws ConnectorException {
		HashMap<String, String> returnValuesMap = new HashMap<String, String>();
		String sMethodName = "getDropValues()";
		logger.setMethodStartLog(className, sMethodName);
		try {
			String sTableNames[] = ((String) hmConstants.get(LOOKUP_BAPI_TABLE_NAME))
					.split(";");
			String sFieldValues[] = ((String) hmConstants.get(LOOKUP_BAPI_FIELDVALUE))
					.split(";");
			String sOffset = ((String) hmConstants.get(OFFSET));
			String sLeng = ((String) hmConstants.get(LENG));
			JCoTable jcoTable = dropDownListFunction.getTableParameterList()
					.getTable(sTableNames[0]);
			JCoTable jcoTable1 = dropDownListFunction.getTableParameterList()
					.getTable(sTableNames[1]);
			...
			logger.debug(className, sMethodName, "jcoTable " +sTableNames[0] 
                              + " Rows: " + iDropDownTableRows);
			if (iDropDownTableRows != 0) {
				for (int iIndex = 0; iIndex < iDropDownTableRows; iIndex++) {
					...
					sField = jcoTable.getString((String) hmConstants
							.get(LOOKUP_BAPI_FIELDNAME));
					if (sField.equals(sFieldValues[0])) {
						iCodeOffSet = Integer.parseInt(jcoTable.getString(sOffset));
						iCodeLength = Integer.parseInt(jcoTable.getString(sLeng));
						iCodeLength = iCodeLength + iCodeOffSet;
					}
					if (sField.equals(sFieldValues[1])) {
						iDescOffSet = Integer.parseInt(jcoTable.getString(sOffset));
						iDescLength = Integer.parseInt(jcoTable.getString(sLeng));
			...
			logger.debug(className, sMethodName, "jcoTable1 " +sTableNames[1] +
					" Rows: " + idropDownHelpValuesTable );
			...
			if (idropDownHelpValuesTable != 0) {
				...
				for (int i = 0; i < idropDownHelpValuesTable; i++) {
					jcoTable1.setRow(i);
					sMessage = jcoTable1.getString((String) hmConstants
							.get(LOOKUP_BAPI_TABLE_HELPVALUES));
					if ((sMessage != null) && (sMessage.length() == 0)) {
						...
						if ((iDescOffSet > 0) && (sMessage.length() > iDescOffSet)) {
					...
					returnValuesMap.put(sCode + "~" + sDecode, sCode + "~" + sDecode);
				}
			}
		} catch (Exception e) {
			throw new ConnectorException(className +" : "+ sMethodName+ " :"  + e.getMessage());
		}
		logger.debug(className, sMethodName, "# of rows in returnValuesMap: " + returnValuesMap.size());
	...
	private JCoFunction getdropDownListFunction(JCoDestination jcoConnection)
		...
		try {
			 ...
			 */
			String sParamFieldNames[] = ((String) hmConstants
					.get(LOOKUP_BAPI_PARAM_FIELD)).split(";");
			String sParamFieldValues[] = ((String) hmConstants
					.get(LOOKUP_BAPI_PARAM_VALUE)).split(";");
			dropDownListFunction = oSAPUtil.getJCOFunction(jcoConnection,
					((String) hmConstants.get(LOOKUP_BAPI_NAME)));
			logger.debug(className, sMethodName, "BAPI name:" + hmConstants.get(LOOKUP_BAPI_NAME));
			
			JCoParameterList parameterList = dropDownListFunction
					.getImportParameterList();
			parameterList.setValue(sParamFieldNames[0], sParamFieldValues[0]);
			logger.debug(className, sMethodName, "Parameter:" 
					+ sParamFieldNames[0] + " set to " + sParamFieldValues[0]);
			parameterList.setValue(sParamFieldNames[1], sParamFieldValues[1]);
			logger.debug(className, sMethodName, "Parameter:" 
					+ sParamFieldNames[1] + " set to " + sParamFieldValues[1]);
			parameterList.setValue(sParamFieldNames[2], sParamFieldValues[2]);
			logger.debug(className, sMethodName, "Parameter:" 
					+ sParamFieldNames[2] + " set to " + sParamFieldValues[2]);
			parameterList.setValue(sParamFieldNames[3], sParamFieldValues[3]);
			logger.debug(className, sMethodName, "Parameter:" 
					+ sParamFieldNames[3] + " set to " + sParamFieldValues[3]);
	...
	private void addLookupvalues(
		...
		try {
			logger.setMethodStartLog(className, sMethodName);
			String sValue = "";
			String sDecode = htHRMSConfig.get(sEmployeeType).toString();
			...
			if (hmLookupUpValues.size() > 0) {
				...
				while (it.hasNext() && !isStopped()) {
					...
					try {
						...
						tclookupIntf.addLookupValue(sLookupCode, sValue.trim(), sDecode,
								"", "");
						...
						logger.setStackTrace(apiEx, className, sMethodName, apiEx
								.getMessage());
						throw new ConnectorException(apiEx.getMessage());
					} catch (tcInvalidLookupException e) {
	...
	private void addLookupCode(String sLookupCode,
		...
		try {
			...
			try {
				...
				throw new ConnectorException("APIException : "+className +" : "+ sMethodName+ " :"  + e1.getMessage());

			} catch (tcDuplicateLookupCodeException e1) {
				logger.error(className, sMethodName, "DuplicateLookupCodeException");
				logger.setStackTrace(e1, className, sMethodName, e1.getMessage());
				throw new ConnectorException("DuplicateLookupCodeException : "+className +" : "+ sMethodName+ " :"  + e1.getMessage());
			}
		} catch (tcDuplicateLookupCodeException e) {
			logger.error(className, sMethodName, "DuplicateLookupCodeException");
			logger.setStackTrace(e, className, sMethodName, e.getMessage());
			throw new ConnectorException("DuplicateLookupCodeException : "+className +" : "+ sMethodName+ " :"  + e.getMessage());
		} catch (Exception e) {
			logger.error(className, sMethodName, "Exception");
			logger.setStackTrace(e, className, sMethodName, e.getMessage());
			throw new ConnectorException(e.getMessage());
